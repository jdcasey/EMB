    ...
    implements MutableBeanLocator
{
    ...
    private final List<Provider<? extends XQualifiedBeans<?, ?>>> exposedBeans =
        new ArrayList<Provider<? extends XQualifiedBeans<?, ?>>>();
    ...
    {
        final XQualifiedBeans<Q, T> beans =
            initialize( null == notify ? new XQualifiedBeans<Q, T>( key ) : new XNotifyingBeans<Q, T>( key, notify ) );
        exposedBeans.add( new XWeakBeanReference<Q, T>( beans ) );
        return beans;
    }
    ...
    public synchronized <Q extends Annotation, T, W> void watch( final Key<T> key, final Mediator<Q, T, W> mediator,
                                                                 final W watcher )
    {
        exposedBeans.add( initialize( new XWatchedBeans<Q, T, W>( key, mediator, watcher ) ) );
    }
    ...
    public synchronized void add( final Injector injector )
    {
        ...
        {
            final XQualifiedBeans<?, ?> beans = exposedBeans.get( i ).get();
            if ( null != beans )
            {
     ...
     */
    private <B extends XQualifiedBeans<?, ?>> B initialize( final B beans )
    {
